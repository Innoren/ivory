"use client"

import { useState } from "react"
import { Button } from "@/components/ui/button"
import { Textarea } from "@/components/ui/textarea"
import { X, Sparkles, Type } from "lucide-react"

interface TextDescriptionDialogProps {
  isOpen: boolean
  onConfirm: (description: string) => void
  onCancel: () => void
}

export function TextDescriptionDialog({
  isOpen,
  onConfirm,
  onCancel
}: TextDescriptionDialogProps) {
  const [description, setDescription] = useState("")

  if (!isOpen) return null

  const handleConfirm = () => {
    if (description.trim()) {
      onConfirm(description.trim())
      setDescription("") // Reset for next time
    }
  }

  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-md flex items-end sm:items-center justify-center z-[100] animate-in fade-in duration-300">
      <div 
        className="bg-white w-full sm:max-w-2xl sm:mx-4 overflow-hidden animate-in slide-in-from-bottom sm:slide-in-from-bottom-4 duration-500 sm:rounded-2xl shadow-2xl"
        style={{
          animation: 'slideUpFade 0.5s cubic-bezier(0.16, 1, 0.3, 1)'
        }}
      >
        {/* Header */}
        <div className="relative bg-gradient-to-br from-[#F8F7F5] to-white px-6 sm:px-8 pt-8 pb-6">
          <button
            onClick={onCancel}
            className="absolute top-4 right-4 w-10 h-10 flex items-center justify-center text-[#6B6B6B] hover:text-[#1A1A1A] hover:bg-white/80 rounded-full transition-all duration-300 active:scale-90"
          >
            <X className="w-5 h-5" strokeWidth={1.5} />
          </button>
          
          <div className="flex flex-col items-center text-center">
            <div className="w-16 h-16 sm:w-20 sm:h-20 bg-gradient-to-br from-[#8B7355] to-[#A0826D] rounded-full flex items-center justify-center mb-4 shadow-lg animate-in zoom-in duration-500 delay-100">
              <Type className="w-8 h-8 sm:w-10 sm:h-10 text-white" strokeWidth={1.5} />
            </div>
            <h3 className="font-serif text-2xl sm:text-3xl font-light text-[#1A1A1A] tracking-tight mb-2 animate-in fade-in slide-in-from-bottom-2 duration-500 delay-200">
              Describe Your Design
            </h3>
            <p className="text-sm sm:text-base text-[#6B6B6B] font-light tracking-wide animate-in fade-in slide-in-from-bottom-2 duration-500 delay-300">
              Describe the nail design you want, then upload a hand photo to see it applied
            </p>
          </div>
        </div>
        
        {/* Content */}
        <div className="px-6 sm:px-8 py-6 sm:py-8">
          <div className="space-y-4">
            <div>
              <label className="text-xs tracking-[0.2em] uppercase text-[#8B7355] font-medium mb-3 block">
                Design Description
              </label>
              <Textarea
                value={description}
                onChange={(e) => setDescription(e.target.value)}
                placeholder="Example: French tips with gold glitter accent on ring finger, almond shape, medium length..."
                className="min-h-[200px] resize-none border-2 border-[#E8E8E8] focus:border-[#8B7355] rounded-xl p-4 text-[#1A1A1A] placeholder:text-[#AFAFAF] font-light transition-all duration-300"
                maxLength={500}
              />
              <div className="flex justify-between items-center mt-2">
                <p className="text-xs text-[#6B6B6B] font-light">
                  Be as detailed as possible for best results
                </p>
                <span className="text-xs text-[#AFAFAF]">
                  {description.length}/500
                </span>
              </div>
            </div>

            {/* Example suggestions */}
            <div className="bg-gradient-to-br from-[#F8F7F5] to-white border border-[#E8E8E8] p-4 rounded-xl">
              <p className="text-xs tracking-[0.2em] uppercase text-[#8B7355] font-medium mb-2">
                Tips for Better Results
              </p>
              <ul className="space-y-1 text-xs text-[#6B6B6B] font-light">
                <li>• Mention colors, patterns, and design elements</li>
                <li>• Specify nail shape (almond, square, stiletto, etc.)</li>
                <li>• Include length preference (short, medium, long)</li>
                <li>• Describe finish (glossy, matte, chrome, etc.)</li>
                <li>• Add style keywords (elegant, bold, minimalist, etc.)</li>
              </ul>
            </div>
          </div>
        </div>
        
        {/* Actions */}
        <div className="px-6 sm:px-8 pb-6 sm:pb-8 flex flex-col sm:flex-row gap-3 animate-in fade-in slide-in-from-bottom duration-500 delay-600">
          <Button
            onClick={onCancel}
            variant="outline"
            className="w-full sm:flex-1 h-14 sm:h-16 border-2 border-[#E8E8E8] hover:border-[#1A1A1A] hover:bg-[#F8F7F5] text-[#1A1A1A] text-sm tracking-[0.2em] uppercase rounded-xl font-light transition-all duration-300 active:scale-95 touch-manipulation"
          >
            Cancel
          </Button>
          <Button
            onClick={handleConfirm}
            disabled={!description.trim()}
            className="w-full sm:flex-1 h-14 sm:h-16 bg-gradient-to-r from-[#1A1A1A] via-[#2D2D2D] to-[#1A1A1A] text-white hover:from-[#8B7355] hover:via-[#A0826D] hover:to-[#8B7355] text-sm tracking-[0.2em] uppercase rounded-xl font-light transition-all duration-500 active:scale-95 hover:shadow-2xl touch-manipulation disabled:opacity-50 disabled:cursor-not-allowed animate-shimmer"
            style={{ backgroundSize: '200% 100%' }}
          >
            <Sparkles className="w-5 h-5 mr-2" strokeWidth={1.5} />
            Use Description
          </Button>
        </div>
      </div>
      
      <style jsx>{`
        @keyframes slideUpFade {
          from {
            opacity: 0;
            transform: translateY(100px) scale(0.95);
          }
          to {
            opacity: 1;
            transform: translateY(0) scale(1);
          }
        }
      `}</style>
    </div>
  )
}
