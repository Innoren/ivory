...
import { Sparkles, Upload, Loader2, X, ChevronDown, Share2, Trash2, Camera, Type } from "lucide-react"
...
import { TextDescriptionDialog } from "@/components/text-description-dialog"
...
export default function CapturePage() {
  ...
  // Check for describe mode in URL - do this BEFORE any other initialization
  const [isDescribeMode, setIsDescribeMode] = useState(false)
  
  // Check for pending description from describe page
  useEffect(() => {
    const pendingDescription = localStorage.getItem("pendingNailDescription")
    if (pendingDescription) {
      setTextDescription(pendingDescription)
      localStorage.removeItem("pendingNailDescription")
      toast.success("Description loaded! Design your nails with: " + pendingDescription.substring(0, 50) + "...")
    }
  }, [])
  ...
  const [showTextDescriptionDialog, setShowTextDescriptionDialog] = useState(false)
  ...
  useEffect(() => {
    const initializePage = async () => {
      // Check for describe mode FIRST
      const params = new URLSearchParams(window.location.search)
      const isInDescribeMode = params.get('mode') === 'describe'
      
      if (isInDescribeMode) {
        console.log('🎨 Describe mode detected - skipping camera initialization')
        setIsDescribeMode(true)
        setIsInitializing(false)
        // Open text description dialog immediately
        setTimeout(() => {
          setShowTextDescriptionDialog(true)
        }, 100)
        // Clean up URL
        window.history.replaceState({}, '', '/capture')
        return // Exit early, don't initialize camera
      }
      ...
      if (savedTabs && !hasLoadedDesignRef.current) {
        try {
          ...
          if (hasContent) {
            ...
            if (!activeTabToRestore.originalImage && activeTabToRestore.finalPreviews.length === 0 && !isDescribeMode) {
              console.log('⚠️ Active tab has no content, starting camera')
              // Add delay to ensure NativeBridge is initialized on iOS
              setTimeout(() => startCamera(), 300)
            } else {
              console.log('✅ Active tab has content or in describe mode, NOT starting camera')
            }
      ...
      if (!capturedImage && !isDescribeMode) {
        console.log('⚠️ No saved tabs and no capturedImage, starting camera')
        // Add delay to ensure NativeBridge is initialized on iOS
        setTimeout(() => startCamera(), 300)
      } else {
        console.log('✅ Have capturedImage or in describe mode, NOT starting camera')
      }
  ...
  const buildPrompt = (settings: DesignSettings) => {
    ...
    // Base prompt with design parameters
    let prompt = `Ultra-detailed, high-resolution nail art design applied ONLY inside a fingernail area. Nail length: ${settings.nailLength}, Nail shape: ${formattedShape}. Base color: ${settings.baseColor}. Finish: ${settings.finish}. Texture: ${settings.texture}. Design style: ${settings.patternType} pattern, ${settings.styleVibe} aesthetic. Accent color: ${settings.accentColor}.`
    
    // Add user's text description if provided
    if (textDescription) {
      prompt += ` USER DESIGN REQUEST: ${textDescription}.`
    }
    
    prompt += ` Highly realistic nail polish appearance: smooth polish, crisp clean edges, even color distribution, professional salon quality with maximum detail, subtle natural reflections. Design must: stay strictly within the nail surface, follow realistic nail curvature, respect nail boundaries, appear physically painted onto the nail with expert precision. Ultra-high resolution rendering, realistic lighting, natural skin reflection preserved, sharp details, vibrant colors with accurate saturation.`
  ...
  if (capturedImage) {
    return (
      <div className="fixed inset-0 z-[100] bg-gradient-to-b from-[#F8F7F5] via-white to-white flex flex-col">
          ...
          <div className="max-w-4xl mx-auto h-full flex flex-col">
            ...
            <div className="w-full flex-1 flex flex-col overflow-hidden">
              <div className="px-4 sm:px-6 lg:px-8 py-4 sm:py-6 space-y-4 sm:space-y-5 overflow-y-auto overscroll-contain flex-1 scrollbar-hide">
                ...
                {/* Option Tabs: Upload or Describe */}
                <div className="flex gap-2 mb-4">
                  <button
                    onClick={() => {/* Already on upload tab */}}
                    className="flex-1 py-3 px-4 border-b-2 border-[#8B7355] text-[#8B7355] text-sm font-medium tracking-wider uppercase transition-all"
                  >
                    Upload Image
                  </button>
                  <button
                    ...
                    className="flex-1 py-3 px-4 border-b-2 border-[#E8E8E8] text-[#6B6B6B] hover:text-[#8B7355] hover:border-[#8B7355] text-sm font-medium tracking-wider uppercase transition-all"
                  >
                    Describe
                  </button>
                </div>
                ...
                {selectedDesignImages.length === 0 && !textDescription && (
                  <div className="text-center py-8">
                    <p className="text-sm text-[#6B6B6B] font-light">No design images uploaded yet</p>
                    <p className="text-xs text-[#6B6B6B] font-light mt-2">Upload a reference image or describe your design</p>
                  </div>
                )}
                ...
                {textDescription && (
                  <div className="space-y-4">
                    <div className="border border-[#E8E8E8] rounded-sm p-4 bg-gradient-to-br from-[#F8F7F5] to-white shadow-sm">
                      <div className="flex items-center justify-between mb-3">
                        <h3 className="text-sm font-light tracking-[0.15em] uppercase text-[#1A1A1A] flex items-center gap-2">
                          <Type className="w-4 h-4 text-[#8B7355]" />
                          Text Description
                        </h3>
                      </div>
                      
                      <div className="bg-white border border-[#E8E8E8] rounded-sm p-3 mb-3">
                      ...
                      <div className="flex gap-2">
                        <button
                          onClick={() => setShowTextDescriptionDialog(true)}
                          className="flex-1 h-10 border border-[#E8E8E8] text-[#1A1A1A] text-xs font-light tracking-[0.15em] uppercase hover:bg-[#F8F7F5] hover:border-[#8B7355] transition-all duration-300 rounded-sm"
                        >
                          Edit
                        </button>
                        <button
                          ...
                          className="flex-1 h-10 border border-red-300 text-red-600 text-xs font-light tracking-[0.15em] uppercase hover:bg-red-50 hover:border-red-400 transition-all duration-300 rounded-sm"
                        >
                          Remove
                        </button>
                      </div>
        ...
        {/* Text Description Dialog */}
        <TextDescriptionDialog
          isOpen={showTextDescriptionDialog}
          onConfirm={(description) => {
            setTextDescription(description)
            setShowTextDescriptionDialog(false)
            toast.success("Perfect! Now upload a photo of your hand to see your design applied.", {
              duration: 5000,
            })
            // Automatically trigger file upload after a short delay
            if (!capturedImage) {
              setTimeout(() => {
          ...
          onCancel={() => {
            setShowTextDescriptionDialog(false)
            // If no image captured yet and no description, go back to home
            if (!capturedImage && !textDescription) {
              router.push('/home')
            }
          }}
        />
